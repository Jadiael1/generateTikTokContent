...
(async () => {
    try {
        ...
        const question = `Crie uma historia onde a mesma comece a primeira linha com uma descrição de no maximo 50 caracteres e no começo da linha deve ter Descrição: seguido da descrição, e nessa descrição deve ter um resumo da historia, e apos a descrição a historia nas proximas linhas, e a historia deve ter 3 minutos baseada em dicas voltadas a nutrição especificamente com HerbaLife, que envolve dicas sobre os produtos da herbalife, dicas de cuidado com o corpo usando os produtos da herbalife.
            A historia deve ser de interesse de mais de 90% da população brasileira e do mundo.
            Vou usar essa historia para postar videos em plataformas de Streaming então a historia deve ser interessante para que eu possa ganhar views.
            Me surpreenda, e me responda em pt-BR
        `;
        ...
        // await generateImages(description, 5, openai, dir);
        await generateImagesP(`nutrition`, 5, config.pexelsapikey, dir);
